...
//insert의 재귀적함수. p에 newElement를 넣는다
{
    ...
    else if ( newElement.level() < p->element.level() )
...

{
    ...
    if ( p == 0 )
       result = 0;
    else if ( searchKey < p->element.level() )
...
//가지고 있는경우에는 cutRightmost를 이용해서 node를 대체하고 cutRightmost를 삭제한다.
{
    ...
    if ( p == 0 )
       result = 0;                              
    else if ( deleteKey < p->element.level() )
...

{
     ...
     if ( p != 0 )
     {
        ...
        for ( j = 0 ; j < level ; j++ )    // Tab over to level
            cout << "\t";
        cout << " " << p->element.level();   // Output key
